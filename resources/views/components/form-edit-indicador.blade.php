    ...
    <div class="row">
        <div class="col-md-6 col-12 mt-3">
            ...
            <select name="nombre" id="nombre" class="form-select" required>
                ...
                <option value="{{ $loop->index }}" {{ $i->nombreIndicador == $indicador->nombreIndicador ? 'selected' :
                    '' }} >{{ $i->nombreIndicador }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-3 col-12 mt-3">
            ...
            <input type="text" class="form-control pe-none" name="unidadMedida" id="unidadMedida"
                placeholder="Unidad de medida" readonly value="{{ $indicador->unidadMedidaIndicador }}">
        </div>

        <div class="col-md-4 col-12 mt-3">
            <label for="" class="form-label">Valor</label>
            <input type="text" class="form-control" name="valor" id="valor" placeholder="Indique el valor" required
                value="{{ $indicador->valorIndicador }}">
            <div class="invalid-feedback">
        ...
        <div class="col-md-4 col-12 mt-3">
            ...
            <input type="date" class="form-control" name="fecha" id="fecha" placeholder="Indique la fecha" required
                value="{{ $indicador->fechaIndicador }}">
            <div class="invalid-feedback">
...
<script>
    ...
    form.submit(function (e) {
        ...
        if (document.getElementById('form').checkValidity()) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ env('APP_URL') }}/api/update-indicador',
                data: {
                    'id': {{ $indicador->id }},
